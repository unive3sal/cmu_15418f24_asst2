...
void usage(const char* progname) {
    ...
    printf("  -m  --test <TYPE>      Run specified function on input.  Valid tests are: scan, "
           "find_peaks\n"); 
...
int cpu_find_peaks(int *start, int length, int *output){
    int count = 0, idx = 1;
    while(idx < length - 1){ 
        if(start[idx-1] < start[idx] && start[idx] > start[idx + 1]){
            output[count] = idx;
            count++;
        }   
        idx++;
    ...
    if (test.compare("scan") == 0) { // test exclusive scan
        ...
        for (int i=0; i<3; i++) {
            cudaTime = std::min(cudaTime,
                            cudaFindPeaks(inarray, N, resultarray, &cu_size));
        }

        // run CPU implementation to check correctness 
        int serial_size = cpu_find_peaks(inarray, N, checkarray);
        ...
        printf("Find_peaks outputs are correct!\n");
