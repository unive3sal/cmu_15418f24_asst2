...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double cuda_kernel_start_time = CycleTimer::currentSeconds();
    // run kernel
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();

    double cuda_kernel_end_time = CycleTimer::currentSeconds();
    ...
    double computation_duration = cuda_kernel_end_time - cuda_kernel_start_time;
    printf("Computation: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * computation_duration, toBW(totalBytes, computation_duration));
